...
(defn- parse-node [line]
  (let [[term k] (str/split line #" -> ")
    ...
    [k (cond
         ...
         :else (throw (ex-info (str "cannot parse line: " line) {:line line})))]))

(defn- parse-graph [spec]
  (into {} (map parse-node (str/split-lines spec))))
                       ...
                       (if (int? x)
                         ...
                         (let [[cmd & args] (get G x)]
                           (case cmd
                             ...
                             :lshift (let [[n k] args] (bit-shift-left (-evaluate G k) n)),
                             :rshift (let [[n k] args] (bit-shift-right (-evaluate G k) n))))))]
